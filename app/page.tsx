...
export default function Page() {
  ...
  // OPTIONAL: Try Gemini TTS (will fail if quota exceeded)
  async function playLatestWithGeminiTTS() {
    const lastBot = [...messages].reverse().find((m) => m.role === "bot");
    if (!lastBot) return alert("No bot message to speak.");
    ...
    try {
      ...
      if (!data.audioBase64) {
        console.error("TTS response:", data);
        // Fallback to browser TTS
        console.log("Gemini TTS failed, using browser TTS instead");
        setIsPlaying(false);
        playLatestWithBrowserTTS();
        return;
      }

      const mimeType = data.mimeType || "audio/wav";
      const audioBlob = b64toBlob(data.audioBase64, mimeType);
