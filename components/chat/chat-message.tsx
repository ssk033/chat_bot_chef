...
export function ChefTypingIndicator({ assistantLabel = "Chef" }: { assistantLabel?: string }) {
  return (
    <div className="message-enter flex w-full gap-3 sm:gap-4">
      <ChefAvatar size={28} />
      <div className="flex min-w-0 max-w-[70%] flex-1 flex-col items-start">
        <span className="mb-1.5 px-0.5 text-[11px] font-medium uppercase tracking-wide text-[var(--muted-text)]">
          {assistantLabel}
        </span>
        <div className="w-full rounded-2xl rounded-bl-md border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-4 py-3 shadow-sm sm:px-5 sm:py-4">
